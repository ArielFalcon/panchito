// Deterministic progress gate for the fix-loop, lifted from src/qa/progress-gate.ts.
// Pure, stateless computation over round evidence — no I/O, no side-effects.
// The service wraps the three public functions (decideProgress, isLikelyRealBug, bestRound)
// as class methods so callers import from one seam (the qa-engine port boundary).
// Parity-pinned: the copied bodies MUST match src/qa/progress-gate.ts until Plan 7 cutover.
...
export class ProgressGateService {
  // Core gate: decides whether to spend a retry slot based on progress signals.
  decideProgress(prev: RoundResult | null, cur: RoundResult): GateDecision {
    return decideProgress(prev, cur);
  }

  // Whether the real-bug branch should fire (all selectors unique + all value-mismatch failures).
  isLikelyRealBug(allSelectorsUnique: boolean, failureDetails: string[]): boolean {
    return isLikelyRealBug(allSelectorsUnique, failureDetails);
  }

  // Selects the best round from a list (fewest failures; ties go to the later round).
  bestRound<T extends { failingCount: number }>(rounds: T[]): T | undefined {
    return bestRound(rounds);
  }
}
