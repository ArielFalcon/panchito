...
const NAMED_SECRET_PATTERNS: Array<{ name: string; p: RegExp; skip?: (m: string) => boolean; modelSkip?: (m: string) => boolean }> = [
  ...
  // api-key-assignment) — kept in lockstep with this file's qa-engine twin (sanitize-text.ts), see
...
// enclosing text and must be re-emitted so the surrounding prose/JSON/markup is not corrupted. Twin
// of qa-engine/src/contexts/generation/infrastructure/sanitize-text.ts's own function — keep in lockstep.
