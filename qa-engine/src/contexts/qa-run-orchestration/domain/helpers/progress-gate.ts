// qa-engine/src/contexts/qa-run-orchestration/domain/helpers/progress-gate.ts
// PORT (VERBATIM, not a rewrite) of src/qa/progress-gate.ts — the deterministic progress gate for
// the fix-loop, consumed by FixLoop (Task D.4, sub-decision 1: the loop condition + progress gate).
//
// Re-homed here rather than imported cross-context: qa-engine ALREADY has a parity-pinned copy of
// these exact functions in test-execution/domain/progress-gate.service.ts, but qa-run-orchestration
// MUST NOT import from a sibling context's domain (the same hexagonal-boundary rule that governs
// adjudicate.service.ts's re-home, and C.2's deriveCycleBackstop re-home before it). A dedicated
// progress-gate-parity.test.ts (added to the qa-engine typecheck "exclude" list) pins THIS copy
// against src/qa/progress-gate.ts directly, independent of test-execution's own parity pin.
//
