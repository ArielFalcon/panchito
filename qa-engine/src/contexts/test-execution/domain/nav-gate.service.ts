...
// Lifted from src/qa/nav-gate.ts. Parity-pinned: the copied bodies MUST match
// src/qa/nav-gate.ts until Plan 7 cutover.
//
...
export class NavGateService {
  // Decides whether navigation to a URL should be allowed on a regen turn.
  decideNav(input: NavGateInput): NavGateDecision {
    return decideNav(input);
  }

  // Reduce a URL to a canonical route path, or null if it cannot be parsed (fail-open).
  normalizeRoute(raw: string): string | null {
    return normalizeRoute(raw);
  }

  // Whether a grounded route template matches a concrete requested route.
  routesMatch(grounded: string, requested: string): boolean {
    return routesMatch(grounded, requested);
  }
}
