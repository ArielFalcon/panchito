...
| **C. Event bus + domain events** | Bus ✅; OpenCode-event → RunEvent mapper ✅ ([activity-mapper.ts](../src/integrations/activity-mapper.ts), `ToolState.title` + all tools + running/completed + todos→plan, fixture-tested against the contract). **Remaining:** wire it (v2 subscribe + v1 fallback) into the bus; `pipeline.onStep` → `step.changed`; stream the Playwright reporter → `test.*`. | Produces the live data. |
