...
//  - The POLICY (off | signal | enforce) decides what a low ratio DOES: signal RECORDS ONLY (the
//    ratio is persisted on the RunOutcome and feeds the learning ledger on subsequent runs — it
//    does NOT reach the in-run reviewer, which has already run by the time coverage is measured);
//    enforce additionally regenerates against the gap and blocks publishing if still low. Default
//    is signal. The e2e provider resolves V8 coverage to repo files two ways: a direct path-suffix
//    match (unbundled/dev), OR — for a bundled/hashed deploy (production Angular/React) whose served
//    URL never matches a repo path — via the script's SOURCE MAP (see source-map.ts), captured by
//    the e2e fixture. A bundle with no source map still degrades to "unknown" (never a false signal).

import { existsSync, readFileSync, readdirSync, rmSync } from "node:fs";
import { join, isAbsolute, relative } from "node:path";
import { TestTarget } from "../types";
import { coveredOriginalLines, type RawSourceMap } from "./source-map";
...
interface V8Entry {
  ...
  // The bundle's source map, captured by the e2e fixture when the served script is a bundle. Lets
  // change-coverage work for a HASHED/BUNDLED deploy (Angular/React prod) whose URL never matches a
  // repo path — the keystone gap (ORACLE-03). Absent for unbundled/dev scripts.
  map?: RawSourceMap;
}
export function parseV8Coverage(entries: V8Entry[], changedFiles: string[]): CoveredLines {
  ...
  for (const entry of entries) {
    ...
    if (directFile) {
      // Unbundled/dev: the served script IS a repo source file — a generated line covered ⇒ that
      // repo line covered. (A line is covered if ANY of its bytes ended up covered.)
      const lines: number[] = [];
      ...
      // Bundled/hashed deploy: the URL is not a repo path. Translate covered BUNDLE positions back
      // to ORIGINAL source files + lines via the source map (ORACLE-03 keystone). A mapping segment
      // counts only if its generated byte actually executed.
    ...
    // else: neither a direct match nor a usable source map → this script yields no coverage signal.
  }
  return out;
}
...
export function defaultCollectCoverage(input: CoverageCollectInput): CoveredLines | null {
  try {
    return input.target === "code"
      ? collectNativeCoverage(input.repoDir, input.changedFiles)
      : collectBrowserCoverage(input.e2eDir, input.changedFiles, input.namespace);
  } catch (err) {
    // Stay fail-safe (return null → "unmeasured", never a false fail), but be LOUD: a collector
    // that THREW (corrupt dump, unreadable report) is "broken", which is operationally different
    // from legitimately finding no data. Swallowing both silently hides a regression in the
    // keystone behind a benign-looking "unknown".
    console.warn(
      `[qa] WARNING: coverage collection (${input.target}) threw — treated as unmeasured, but this is a BROKEN collector, not "no coverage data": ${err instanceof Error ? err.message : String(err)}`,
    );
    return null;
  }
}

function collectNativeCoverage(repoDir: string, changedFiles: string[] = []): CoveredLines | null {
  // lcov first (universal: c8/nyc/pytest-cov/go), then Istanbul JSON (Node default), then JaCoCo
  // XML (JVM: Maven/Gradle — covers the Java/Spring target when the repo has the JaCoCo plugin).
  ...
  for (const rel of jacocoPaths) {
    const p = join(repoDir, rel);
    if (existsSync(p)) {
      const cov = parseJacocoXml(readFileSync(p, "utf8"), changedFiles);
      if (cov.size) return cov;
    }
  }
  return null;
...
// Parse a JaCoCo XML report into covered lines. JaCoCo keys sources by package + filename
// (e.g. "com/example/Foo.java"); resolve each to the matching CHANGED repo file by path suffix
// (the source root — src/main/java, etc. — is not in the report). A line with ci>0 (covered
// instructions) is covered. Regex-based: JaCoCo XML is flat and the orchestrator ships no XML lib.
export function parseJacocoXml(xml: string, changedFiles: string[]): CoveredLines {
  const out: CoveredLines = new Map();
