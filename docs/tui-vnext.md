...
| **C. Event bus + domain events** | Bus ✅; OpenCode-event → RunEvent mapper ✅ ([activity-mapper.ts](../src/integrations/activity-mapper.ts)); Playwright stream → `test.*` mapper ✅ ([test-events.ts](../src/qa/test-events.ts), reporter enriched with per-test duration). **Remaining:** `pipeline.onStep` → `step.changed`; wire all producers into the bus via a v2 `event.subscribe` adapter with v1 fallback. | Produces the live data. |
