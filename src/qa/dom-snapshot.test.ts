...
import { buildChangedMarker } from "./dom-snapshot";
...
test("FIX-1 end-to-end: guidance 'test the contact form' must NOT mark test-submission or form-details nodes", async () => {
  const { changedElementsFromGuidance } = await import("./changed-elements");
  const changed = changedElementsFromGuidance("test the contact form");
