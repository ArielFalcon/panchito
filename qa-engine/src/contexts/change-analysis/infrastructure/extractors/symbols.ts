// Tree-sitter WASM-based symbol extractor. Parses source files deterministically so the agent
// receives structured symbol+signature data without Serena round-trips.
// Ported from src/qa/static-signal/symbols.ts (Plan 7.3 §2 — sever the qa-engine→src/ import
// edge): same behavior, byte-for-byte, using qa-engine's own LanguageRegistry instead of the
// legacy languages.ts. Parser cache is exported for reuse by relations.ts (mirrors the legacy
// design intent — "exported for reuse by the relations extractor").
...
// LanguageRegistry maps .tsx → "typescript", so one grammar covers both.
...
/**
 * Expose the parser cache for the relations extractor so it can reuse already-loaded
 * grammars without paying the WASM load cost again.
 */
