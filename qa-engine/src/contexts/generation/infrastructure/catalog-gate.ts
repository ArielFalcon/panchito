// qa-engine/src/contexts/generation/infrastructure/catalog-gate.ts
// PORT (verbatim) of src/qa/catalog-gate.ts — Pillar 2, the confidence-aware selector gate
...
// 1+3 carry the hard cases). PURE — composes the spec analysis (selector-catalog-window) with the
// captured RouteCatalog (route-catalog); no browser, no I/O, fully unit-testable.
//
...
//
// Consumes confidentWindowEnd + extractTestIdSelectorsWithIndex from the co-located
// selector-catalog-window.ts (NOT re-imported from either qa-engine selector-check copy — see that
// file's header for the hexagonal-boundary + private-helper reasons).
