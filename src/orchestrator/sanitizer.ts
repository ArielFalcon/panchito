...
const NAMED_SECRET_PATTERNS: Array<{ name: string; p: RegExp; skip?: (m: string) => boolean; modelSkip?: (m: string) => boolean }> = [
  ...
  // this same trailing-`\S+` shape (generic-credential, env-credential, api-key-assignment) — kept
  // in lockstep with this file's qa-engine twin (sanitize-text.ts), see sanitize-text-parity.test.ts.
